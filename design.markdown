...
- 判斷用戶的認知負擔、情緒及意圖非常困難，有兩種主要作法：
  ...
  - 情緒照顧的「溫和詢問」：最能照顧使用者（原則 3, 原則 7）比穩定性（原則 8）重要；制式化的「溫柔」恐怕效果更差
...
- 資訊呈現方式：原則 6 要求熟悉工具的使用者（老手）能儘快得到分析結果。最快的結果就是一次互動就拿到詳細分析結果或開始角色扮演。因此：
...
- 高壓力安全流程（原則 2）：此安全流程中，必須追求指令穩定性。
  - 在確認用戶身心狀態以及意圖時，根據原則 2, 必須謹慎確認用戶真的想要 AI 的答案。只要用戶沒有明確表示，就代表不要。為了強制 LLM 做到這點（防止 LLM 被用戶的元問題或元對話帶跑），可以犧牲原則 8-9，用冗餘重複的指令強迫 LLM 忽略用戶的元對話（如「什麼意思」、「都可以」）。
